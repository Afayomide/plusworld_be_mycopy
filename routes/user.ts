...
function verifyToken(req:any, res:any, next:any) {
  ...
  router.put("/", verifyToken,upload.single('profileImage'), async (req:any, res:any) => {
    ...
    updateData.profileImage = profileImageUrl; // Store the S3 URL in the profile
      ...
      const allFieldsFilled = fullname && username && updateData.profileImage && email && phone && hobbies && location && language && goal;
    ...
    router.post('/cart/add', verifyToken, async (req:any, res:any) => {
        ...
        router.post('/multiplecart/add', verifyToken, async (req: any, res: any) => {
          const userId = req.user.userId;
          try {
            ...
            if (coursesToAdd.length > 0) {
              ...
              return res.status(400).json({ message: 'All courses are already in the cart' });
            }
        ...
        router.delete('/cart/remove', verifyToken, async (req: any, res: any): Promise<void> => {
          const userId = req.user?.userId;
        ...
        router.get('/cart/list', verifyToken, async (req:any, res:any) => {
          const id = req.user.userId; 
        ...
        router.get('/courseprogress/:courseId',verifyToken, async (req:any, res:any) => {
          const userId = req.user.userId
          try {
            ...
            if (!courseProgress) {
              return res.status(404).json({ message: 'Course not found in user\'s course list' });
            }
        ...
        router.get('/courses/:courseId/lesson/:lessonId', verifyToken, async (req: any, res: any) => {
          const userId = req.user.userId;
          ...
          try {
            ...
            const currentLesson = lessons.find(lesson => lesson._id.toString() === lessonId);
              ...
              console.error(error);
              return res.status(500).json({ message: 'An error occurred while retrieving the lesson', error: error.message });
          }
      });

      router.post('/courses/:courseId/lesson/:lessonId/complete', verifyToken, async (req: any, res: any) => {
        const userId = req.user.userId;
        const { courseId, lessonId } = req.params;
        ...
        try {
          ...
          const currentLesson = currentModule?.lessons.find(lesson => lesson._id.toString() === lessonId);
          ...
          if (userLesson) {
            ...
            return res.status(404).json({ message: 'Lesson not found in user\'s course progress' });
          }
          ...
          await user.save();
          return res.json({ message: 'Lesson marked as completed and next lesson enabled' });
      
        } catch (error: any) {
          console.error(error);
          return res.status(500).json({ message: 'An error occurred while completing the lesson', error: error.message });
        }
      });
        ...
        router.get("/paidcourses", verifyToken, async (req:any, res:any) =>{
          const userId = req.user.userId
          try{
                      const user = await User.findById(userId) 
             if(!user) {
        ...
        router.get("/paidcourses/:courseId", verifyToken, async (req: any, res: any) => {
          const userId = req.user.userId;
          const { courseId } = req.params;
          ...
          try {
            ...
            if (!userCourse) {
              return res.status(404).json({ message: "Course not found in user's paid courses" });
            }
              ...
              courseModules: course.courseModules?.map(module => ({
                ...
                lessons: module.lessons.map(lesson => {
                  ...
                  return {
                    ...
                    completed: userLessonProgress ? userLessonProgress.completed : false,
                    isOpened: userLessonProgress ? userLessonProgress.isOpened : false,
                    isEnabled: userLessonProgress ? userLessonProgress.isEnabled: false
                  };
                }),
              })),
            };
